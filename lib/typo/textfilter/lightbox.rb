...
* **alt** The alt text associated with the image.  By default, this is the same as the title.
      ...
      def self.macrofilter(blog,content,attrib,params,text="")
        ...
        %{<div style="#{style}" class="lightboxplugin"><a href="#{displayurl}" rel="lightbox" title="#{title}"><img src="#{thumburl}" #{%{width="#{width}" } unless width.nil?}#{%{height="#{height}" } unless height.nil?}alt="#{alt}" title="#{title}"/></a>#{captioncode}</div>}.html_safe()
      end

      def self.set_whiteboard(blog, content)
        content.whiteboard['page_header_lightbox'] = "          <link href=\"#{PATH_PREFIX}/stylesheets/lightbox.css\" media=\"all\" rel=\"Stylesheet\" type=\"text/css\" />\n          <script src=\"#{PATH_PREFIX}/javascripts/lightbox.js\" type=\"text/javascript\"></script>"
      end
    end
  end
end
